...
import requests
import config
from config import BOT_TOKEN, API_HASH, API_ID, OWNER_ID
...
try:
	open(f"sudo{bot_id}.txt","r")
except FileNotFoundError:
	open(f"sudo{bot_id}.txt","w")
try:
	open(f"maindevs{bot_id}.txt","r")
except FileNotFoundError:
	open(f"maindevs{bot_id}.txt","w")
try:
	open(f"maindevsVII{bot_id}.txt","r")
except FileNotFoundError:
	open(f"maindevsVII{bot_id}.txt","w")
try:
	open(f"groups{bot_id}.txt","r")
except FileNotFoundError:
	open(f"groups{bot_id}.txt","w")
try:
	open(f"band{bot_id}.txt","r")
except FileNotFoundError:
	open(f"band{bot_id}.txt","w")
try:
	open(f"links{bot_id}.txt","r")
except FileNotFoundError:
	open(f"links{bot_id}.txt","w")
try:
	open(f"channel{bot_id}.txt","r")
except FileNotFoundError:
	open(f"channel{bot_id}.txt","w")
try:
	open(f"devchannel{bot_id}.txt","r")
except FileNotFoundError:
	open(f"devchannel{bot_id}.txt","w")
try:
	open(f"devuser{bot_id}.txt","r")
except FileNotFoundError:
	open(f"devuser{bot_id}.txt","w")
try:
	open(f'owner{bot_id}.txt','r')
except FileNotFoundError:
	f = open(f'owner{bot_id}.txt','w')
	f.write(str(owner))
...
def is_user(id):
	result = False
	file = open(f"Users{bot_id}.txt","r")
	for line in file:
		if line.strip()==id:
			result = True
	file.close()
...
def is_main_dev(id):
	result = False
	file = open(f"maindevs{bot_id}.txt","r")
	for line in file:
		if line.strip()==id:
			result = True
	file.close()
...
def is_main_devVII(id):
	result = False
	file = open(f"maindevsVII{bot_id}.txt","r")
	for line in file:
		if line.strip()==id:
			result = True
	file.close()
...
def is_group(id):
	result = False
	file = open(f"groups{bot_id}.txt","r")
	for line in file:
		if line.strip()==id:
			result = True
	file.close()
...
def show_channel() -> str:
	with open(f"channel{bot_id}.txt","r") as file:
		return file.readline()

def add_channel(chat_id):
	with open(f"channel{bot_id}.txt","w") as file:
		file.write(chat_id)
...
def get_bot_owner() -> int:
	with open("owner{bot_id}.txt","r") as file:
		return file.readline()
		
def set_bot_owner(user_id:int):
	with open(f"owner{bot_id}.txt","w") as file:
		file.write(str(user_id))

def show_devchannel() -> str:
	with open(f"devchannel{bot_id}.txt","r") as file:
		return file.readline()

def add_devchannel(chat_id):
	with open(f"devchannel{bot_id}.txt","w") as file:
		file.write(chat_id)
...
def show_devuser() -> str:
	with open(f"devuser{bot_id}.txt","r") as file:
		return file.readline()

def add_devuser(chat_id):
	with open(f"devuser{bot_id}.txt","w") as file:
		file.write(chat_id)
...
async def app_start(c:Client,m:Message):
	do = requests.get(f"https://api.telegram.org/bot{BOT_TOKEN}/getChatMember?chat_id=@{show_channel()}&user_id={m.from_user.id}").text
	user = m.from_user.id
	mm = m.from_user.mention
	mainSudo = open(f"maindevs{bot_id}.txt","r").read()
	mainSudoVII = open(f"maindevsVII{bot_id}.txt","r").read()
	mainSudoVII = open(f"maindevsVII{bot_id}.txt","r").read()
	Sudo = open(f"sudo{bot_id}.txt","r").read()
	banD = open(f"band{bot_id}.txt","r").read()
...
async def __count(c:Client,m:Message):
	...
	if  str(user) in mainSudo or str(user) in sudo or str(user) in mainSudoVII or int(user) == owner:
		users = len(open(f"Users{bot_id}.txt","r").readlines())
		groups = len(open(f"groups{bot_id}.txt","r").readlines())
		sudos = len(open(f"sudo{bot_id}.txt","r").readlines())
		main = len(open(f"maindevs{bot_id}.txt","r").readlines())
		bans = len(open(f"band{bot_id}.txt","r").readlines())
...
async def __get_copy(c:Client,m:Message):
	...
	if str(user) in mainSudo or str(user) in sudo or str(user) in mainSudoVII or int(user) == owner:
		users = open(f"Users{bot_id}.txt","rb")
		groups = open(f"groups{bot_id}.txt","rb")
		band = open(f"band{bot_id}.txt","rb")
		sudos = open(f"sudo{bot_id}.txt","rb")
		main = open(f"maindevs{bot_id}.txt","rb")
		
		uc = len(open(f"Users{bot_id}.txt","r").readlines())
		gc = len(open(f"groups{bot_id}.txt","r").readlines())
		bc = len(open(f"band{bot_id}.txt","r").readlines())
		sc = len(open(f"sudo{bot_id}.txt","r").readlines())
		mc = len(open(f"maindevs{bot_id}.txt","r").readlines())
...
async def show_groups(c:Client,m:Message):
	...
	if str(user) in mainSudo or str(user) in sudo or str(user) in mainSudoVII or int(user) == owner:
		groups = open(f"groups{bot_id}.txt")
		x = 1
		text = "**Bot groups **:\n\n"
		for gr in groups:
			text += f"{x} - {gr} \n"
		...
		time.sleep(.5)
		leng = len(open(f"groups{bot_id}.txt","r").readlines())
		if leng == 0:
...
async def __gcopy(c:Client,m:Message):
	...
	if str(user) in mainSudo or str(user) in sudo or str(user) in mainSudoVII or int(user) == owner:
		gr = open(f"groups{bot_id}.txt","rb")
		gc = len(open(f"groups{bot_id}.txt","r").readlines())
...
async def show_links(c:Client,m:Message):
	...
	if str(user) in mainSudo or str(user) in sudo or str(user) in mainSudoVII or int(user) == owner:
		x = 1
		text = "**Groups links **:\n\n"
		lenl = len(open(f"links{bot_id}.txt","r").readlines())
		sl = open(f"links{bot_id}.txt","r")
		for link in sl:
			text += f"{x} - {link}"
			x += 1
...
async def get_copy___band(c:Client,m:Message):
	...
	if str(user) in mainSudo or str(user) in sudo or str(user) in mainSudoVII or int(user) == owner:
		file = open(f"band{bot_id}.txt","rb")
		lenb = len(open(f"band{bot_id}.txt","r").readlines())
...
async def countofuserBan(c:Client,m:Message):
	...
	if str(user) in mainSudo or str(user) in sudo or str(user) in mainSudoVII or int(user) == owner:
		...
		lens = len(open(f"band{bot_id}.txt","r").readlines())
		time.sleep(.5)
		if lens == 0:
...
async def ShowMain(c:Client,m:Message):
	...
	if str(user) in mainSudo or str(user) in mainSudoVII or int(user) == owner:
		file = open(f"maindevs{bot_id}.txt","r")
		lens = len(open(f"maindevs{bot_id}.txt","r").readlines())
...
async def get_MainSudo(c:Client,m:Message):
	...
	if str(user) in mainSudo or str(user) in mainSudoVII or int(user) == owner:
		file = open(f"maindevs{bot_id}.txt","rb")
		lenb = len(open(f"maindevs{bot_id}.txt","r").readlines())
...
async def countofDev(c:Client,m:Message):
	...
	if str(user) in mainSudo or str(user) in mainSudoVII or int(user) == owner:
		...
		lens = len(open(f"maindevsVII{bot_id}.txt","r").readlines())
		time.sleep(.5)
		if lens == 0:
...
async def __show_sudos(c:Client,m:Message):
	...
	if str(user) in mainSudo or str(user) in sudo or str(user) in mainSudoVII or int(user) == owner:
		file = open(f"sudo{bot_id}.txt","r")
		lens = len(open(f"sudo{bot_id}.txt","r").readlines())
...
async def get_copy_Sudo(c:Client,m:Message):
	...
	if str(user) in mainSudo or str(user) in sudo or str(user) in mainSudoVII or int(user) == owner:
		file = open(f"sudo{bot_id}.txt","rb")
		lenb = len(open(f"sudo{bot_id}.txt","r").readlines())
...
async def show_users(c:Client,m:Message):
	...
	if str(user) in mainSudo or str(user) in sudo or str(user) in mainSudoVII or int(user) == owner:
		...
		time.sleep(.5)
		lenm = len(open(f"Users{bot_id}.txt","r").readlines())
		if lenm == 0:
...
async def __get_users_copy(c:Client,m:Message):
	...
	if str(user) in mainSudo or str(user) in sudo or str(user) in mainSudoVII or int(user) == owner:
		...
		time.sleep(2)
		lenu = len(open(f"Users{bot_id}.txt","r").readlines())
		users = open(f"Users{bot_id}.txt","rb")
		if lenu == 0:
...
async def countofusers(c:Client,m:Message):
	...
	if str(user) in mainSudo or str(user) in sudo or str(user) in mainSudoVII or int(user) == owner:
		l = await m.reply("**◍ جاري حساب عدد الاعضاء √**")
		lens = len(open(f"Users{bot_id}.txt","r").readlines())
		time.sleep(.5)
		if lens == 0:
			return await l.edit("**◍ لم يدخل اي عضو للبوت حتي الآن √**")
		return await l.edit(f"**◍ عدد اعضاء بوتك {lens} √**")
	...
	if int(user) == owner:
		...
		if ask.text == "الغاء":
			...
			if(not is_main_dev(str(inputText))):
				AddMain2 = open(f"maindevsVII{bot_id}.txt","a")
				AddMain2.write("{}\n".format(inputText))
	...
	if int(user) == owner:
		...
		if inputText =="الغاء":
			...
			await m.reply("**تم الالغاء**")
				...
				with open(f"maindevsVII{bot_id}.txt","r") as w:
					lines = w.readlines()
				with open(f"maindevsVII{bot_id}.txt","w") as w:
					for line in lines:
						if line.strip("\n")!="{}".format(inputText):
							w.write(line)
...
async def AddMain(c:Client,m:Message):
	...
	if str(user) in mainSudo or str(user) in mainSudoVII or int(user) == owner:
		...
		if ask.text == "الغاء":
			...
			inputText = ask.text
			if(not is_main_dev(str(inputText))):
				AddMain2 = open(f"maindevs{bot_id}.txt","a")
				AddMain2.write("{}\n".format(inputText))
	...
	if str(user) in mainSudo or str(user) in mainSudoVII or int(user) == owner:
		...
		if inputText =="الغاء":
			...
			await m.reply("**تم الالغاء**")
				...
				with open(f"maindevs{bot_id}.txt","r") as w:
					lines = w.readlines()
				with open(f"maindevs{bot_id}.txt","w") as w:
					for line in lines:
						if line.strip("\n")!="{}".format(inputText):
							w.write(line)
...
async def AddSudo(c:Client,m:Message):
	...
	if str(user) in mainSudo or str(user) in mainSudoVII or int(user) == owner:
		...
		if ask.text == "الغاء":
			...
			inputText = ask.text
			if(not is_main_dev(str(inputText))):
				AddMain2 = open(f"sudo{bot_id}.txt","a")
				AddMain2.write("{}\n".format(inputText))
	...
	if str(user) in mainSudo or str(user) in mainSudoVII or int(user) == owner:
		...
		if inputText =="الغاء":
			...
			await m.reply("**تم الالغاء**")
				...
				with open(f"sudo{bot_id}.txt","r") as w:
					lines = w.readlines()
				with open(f"sudo{bot_id}.txt","w") as w:
					for line in lines:
						if line.strip("\n")!="{}".format(inputText):
							w.write(line)
	 ...
	 if inputText == "الغاء":
	 	...
	 	await m.reply("**تم الالغاء**")
			...
			if(not is_band(str(inputText))):
				AddBan = open(f"band{bot_id}.txt","a")
				AddBan.write("{}\n".format(inputText))
...
async def AllCommand__(c,m):
	...
	if str(user) in mainSudo or str(user) in sudo or str(user) in mainSudoVII or int(user) == owner:
		...
		if inputText == "الغاء":
			...
			users = open(f"Users{bot_id}.txt","r")
			groups = open(f"groups{bot_id}.txt","r")
			bans = open(f"band{bot_id}.txt","r")
...
url=f"https://t.me/{show_devchannel()}")]])
			...
			x1 = len(open(f"Users{bot_id}.txt","r").readlines())
			x2 = len(open(f"band{bot_id}.txt","r").readlines())
			x3 = len(open(f"groups{bot_id}.txt","r").readlines())
...
url=f"https://t.me/{show_devchannel()}")]])
			...
			us = len(open(f"Users{bot_id}.txt","r").readlines())
...
url=f"https://t.me/{show_devchannel()}")]])
			...
			bn = len(open(f"band{bot_id}.txt","r").readlines())
			await app.send_message(chat,f"**تم الاذاعه الي **: \n {bn} من المحظورين")
...
async def memcommands__(c,m):
	...
	if str(user) in mainSudo or str(user) in sudo or str(user) in mainSudoVII or int(user) == owner:
		...
		if inputText == "الغاء":
			...
			group = open(f"groups{bot_id}.txt","r")
...
url=f"https://t.me/{show_devchannel()}")]])
			...
			gr = len(open(f"groups{bot_id}.txt","r").readlines())
			await app.send_message(chat,f"**تم الاذاعه الي **: \n {gr} من الجروبات")
